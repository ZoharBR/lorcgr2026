#!/bin/bash
# ========================================
# DEPLOY COMPLETO - Correções de Auditoria
# ========================================
# 1. Tecla Delete funcionando no terminal
# 2. Gravação completa de sessões SSH
# 3. Visualização e download de sessões
# 4. Delete de logs para admin
# ========================================

set -e

echo "========================================"
echo "DEPLOY - Correções de Auditoria"
echo "========================================"
...
# ========================================
# 1. CRIAR TABELAS DE AUDITORIA
# ========================================
echo "[1/6] Criando tabelas de auditoria..."
...
def delete_audit_log(request, log_id):
    ...
    try:
        ...
        # Verificar se existe
        cur.execute("SELECT id, action, details FROM audit_logs WHERE id = %s", [log_id])
        row = cur.fetchone()
        if not row:
            cur.close()
            conn.close()
        ...
        response = JsonResponse({'success': True, 'message': 'Log excluido com sucesso'})
        response['Access-Control-Allow-Origin'] = '*'
        return response
...
echo "    ✓ App audit criado"

# ========================================
# 3. ATUALIZAR TERMINAL CONSUMER
# ========================================
echo "[3/6] Atualizando terminal consumer..."
...
class SSHTerminalConsumer(AsyncWebsocketConsumer):
    ...
    def set_terminal_modes(self):
        ...
        try:
            ...
            # Configurar terminal como xterm-256color
            os.environ['TERM'] = 'xterm-256color'

            # Configurações para teclas especiais funcionarem
            # ICRNL: CR para NL no input
            # IXON: Desabilita flow control (Ctrl+S/Q)
            mode[0] = mode[0] | termios.ICRNL
            mode[0] = mode[0] & ~termios.IGNCR
            
            # OPOST: Habilita output processing
            # ONLCR: NL para CR-NL no output
            mode[1] = mode[1] | termios.OPOST | termios.ONLCR
            
            # ECHO: Echo local
            # ECHOE: Echo erase
            # ECHOK: Echo kill
            # ICANON: Canonical mode (permite backspace funcionar)
            mode[2] = mode[2] | termios.ECHO | termios.ECHOE | termios.ECHOK
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            if data.get('type') == 'input':
                ...
                # Capturar comandos (quando Enter é pressionado)
                if char == '\r' or char == '\n':
                    if self.current_line.strip():
                        # Registrar comando executado
                        await self.log_audit('COMMAND', f'Comando: {self.current_line.strip()}')
                        self.commands_executed += self.current_line.strip() + '\n'
                    self.current_line = ""
    ...
    async def get_device_credentials(self):
        """Buscar credenciais do dispositivo"""
        loop = asyncio.get_event_loop()
        future = loop.run_in_executor(None, self._get_device_sync)
        return await future
...
echo "    ✓ Terminal consumer atualizado"

# ========================================
# 4. REGISTRAR APP AUDIT NO DJANGO
# ========================================
echo "[4/6] Registrando app audit no Django..."

# Adicionar app audit ao INSTALLED_APPS
if ! grep -q "'audit'" /opt/lorcgr/lorcgr_core/settings.py; then
    sed -i "/INSTALLED_APPS = \[/,/\]/s/\]/    'audit',\n]/" /opt/lorcgr/lorcgr_core/settings.py
fi

# Adicionar URLs de auditoria
if ! grep -q "audit" /opt/lorcgr/lorcgr_core/urls.py 2>/dev/null; then
    # Garantir que o import include existe
    if ! grep -q "from django.urls import.*include" /opt/lorcgr/lorcgr_core/urls.py; then
        sed -i 's/from django.urls import path/from django.urls import path, include/' /opt/lorcgr/lorcgr_core/urls.py
    fi
    
    # Adicionar rota de audit
    sed -i "/urlpatterns = \[/a\    path('api/audit/', include('audit.urls'))," /opt/lorcgr/lorcgr_core/urls.py
fi

echo "    ✓ App audit registrado"

# ========================================
# 5. DEPLOYAR NOVO FRONTEND
# ========================================
echo "[5/6] Deployando novo frontend..."

# URL do frontend (será atualizada pelo usuário)
FRONTEND_URL="${1:-}"

if [ -n "$FRONTEND_URL" ]; then
    cd /tmp
    curl -sL "$FRONTEND_URL" -o lorcgr-frontend.tar.gz
    
    systemctl stop lorcgr-frontend
    
    cd /opt/lorcgr-frontend
    rm -rf .next node_modules
    tar -xzf /tmp/lorcgr-frontend.tar.gz --strip-components=1
...
ENV
    ...
    echo "    ! URL do frontend não fornecida, pulando..."
fi
...
echo "  ✓ Tecla Delete funcionando no terminal"
echo "  ✓ Sessões sendo gravadas no banco"
echo "  ✓ API de auditoria com sessões"
echo "  ✓ Delete de logs para admin"
echo ""
echo "Endpoints disponíveis:"
echo "  GET  /api/audit/logs/              - Listar logs"
echo "  DELETE /api/audit/logs/<id>/delete/ - Deletar log"
echo "  GET  /api/audit/sessions/          - Listar sessões"
echo "  GET  /api/audit/sessions/<id>/     - Ver conteúdo"
echo "  GET  /api/audit/sessions/<id>/download/ - Baixar TXT"
echo ""
echo "Para testar:"
echo "  curl http://45.71.242.131:8000/api/audit/logs/"
echo "  curl http://45.71.242.131:8000/api/audit/sessions/"
echo ""
