...
const Dashboard = () => {
  ...
  const [alarmConfig, setAlarmConfig] = useState({ temp_warning: 45, temp_critical: 60, rx_warning: -20, rx_critical: -25, tx_warning: 0, tx_critical: -5 });
  ...
  const getStatusTextColor = (s) => s === "critical" ? "text-red-500" : s === "warning" ? "text-yellow-500" : s === "down" ? "text-red-700" : s === "normal" ? "text-green-500" : "text-gray-500";

  const COLORS = { online: "#22c55e", offline: "#ef4444", normal: "#22c55e", warning: "#eab308", critical: "#ef4444", down: "#7f1d1d" };

  const devicePieData = [
    { name: "Online", value: onlineDevices, color: COLORS.online },
    { name: "Offline", value: offlineDevices, color: COLORS.offline }
  ].filter(d => d.value > 0);

  const gbicPieData = [
    { name: "Normal", value: gbicSummary.normal, color: COLORS.normal },
    { name: "Atencao", value: gbicSummary.warning, color: COLORS.warning },
    { name: "Critico", value: gbicSummary.critical, color: COLORS.critical },
    { name: "Down", value: gbicSummary.down, color: COLORS.down }
  ].filter(d => d.value > 0);
            ...
            <table className="w-full">
              <thead className="bg-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-center text-sm">Temp (C)</th>
                  <th className="px-4 py-3 text-center text-sm">TX (dBm)</th>
                  <th className="px-4 py-3 text-center text-sm">RX (dBm)</th>
                ...
                {filteredGbics.map((gbic, idx) => (
                  ...
                  >
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-gray-500">{gbic.device_ip}</div>
                    </td>
                    <td className="px-4 py-3">
                      <div>{gbic.interface}</div>
                      {gbic.description && <div className="text-xs text-gray-500">{gbic.description}</div>}
                    </td>
        ...
        <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4" onClick={() => setSelectedGbic(null)}>
          <div className="bg-gray-800 rounded-lg p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <div className="grid grid-cols-2 gap-4 mb-6">
              ...
              <div>
                <label className="text-gray-400 text-sm">IP</label>
                <div>{selectedGbic.device_ip}</div>
              </div>
              <div>
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-gray-700 rounded p-4 text-center">
                ...
                <div className={`text-2xl font-bold ${selectedGbic.temperature > 45 ? 'text-red-400' : 'text-green-400'}`}>
                  {formatValue(selectedGbic.temperature, ' C')}
                </div>
              </div>
              <div className="bg-gray-700 rounded p-4 text-center">
                ...
                <div className={`text-2xl font-bold ${selectedGbic.tx_power < 0 ? 'text-yellow-400' : 'text-green-400'}`}>
                  {formatValue(selectedGbic.tx_power, ' dBm')}
                </div>
              </div>
              <div className="bg-gray-700 rounded p-4 text-center">
                ...
                <div className={`text-2xl font-bold ${selectedGbic.rx_power < -20 ? 'text-red-400' : 'text-green-400'}`}>
                  {formatValue(selectedGbic.rx_power, ' dBm')}
                </div>
              </div>
            </div>
