#!/bin/bash
# ========================================
# DEPLOY COMPLETO - Terminal SSH WebSocket
# + Credenciais SSH/SNMP
# LOR-CGR Dashboard
# ========================================

set -e

FRONTEND_URL="https://files.catbox.moe/x7mpvy.gz"
STATIC_URL="https://files.catbox.moe/f8njaa.gz"

echo "========================================"
echo "DEPLOY COMPLETO - Terminal SSH WebSocket"
...
class SSHTerminalConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if not device:
            await self.send(text_data=json.dumps({
                'type': 'error',
                'data': '\x1b[31mErro: Dispositivo nao encontrado\x1b[0m\r\n'
            }))
            await self.close()
            return
        ...
        try:
            ...
            await self.send(text_data=json.dumps({
                'type': 'connected',
                'data': f'\x1b[32m OK Conectado a {device["name"]} ({device["ip"]})\x1b[0m\r\n'
            }))
            ...
            await self.send(text_data=json.dumps({
                'type': 'error',
                'data': '\x1b[31m Falha na autenticacao SSH. Verifique usuario e senha.\x1b[0m\r\n'
            }))
            ...
            await self.send(text_data=json.dumps({
                'type': 'error',
                'data': f'\x1b[31m Erro de conexao: {str(e)}\x1b[0m\r\n'
            }))
        ...
        while self.connected:
            try:
                ...
                if self.channel.closed:
                    ...
                    await self.send(text_data=json.dumps({
                        'type': 'disconnected',
                        'data': '\r\n\x1b[33mConexao SSH encerrada\x1b[0m\r\n'
                    }))
                    break
            except Exception as e:
...
echo "[7] Atualizando settings.py..."
...
# 8. Atualizar views_simple.py com TODOS os campos de credenciais
echo "[8] Atualizando API de dispositivos..."
...
def api_save_device(request):
    """Salvar dispositivo COM TODAS AS CREDENCIAIS"""
    try:
...
# 9. Criar serviço systemd para Daphne (WebSocket)
echo "[9] Criando serviço Daphne..."
...
# 11. Baixar e instalar novo frontend
echo "[11] Baixando novo frontend..."
...
# 12. Reiniciar todos os serviços
echo "[12] Reiniciando servicos..."
...
# 14. Testar APIs
echo ""
echo "[14] Testando APIs..."
echo "Devices com credenciais:"
curl -s http://127.0.0.1:8000/api/devices/list/ | python3 -c "
import sys,json
d=json.load(sys.stdin)
print(f'Total: {len(d)} dispositivos')
for dev in d[:3]:
    print(f\"  - {dev['hostname']}: SSH user={dev.get('ssh_user','N/A')}, SNMP comm={dev.get('snmp_community','N/A')}\")"
...
echo "  - WebSocket SSH:   ws://45.71.242.131/ws/terminal/<device_id>/"
echo "  - Next.js:         http://45.71.242.131/"
echo ""
echo "FUNCIONALIDADES:"
echo "  ✓ Terminal SSH em tempo real via WebSocket"
echo "  ✓ Sistema de abas para múltiplas conexões"
echo "  ✓ Credenciais SSH/SNMP salvas no banco"
echo "  ✓ Input interativo (cada tecla enviada em tempo real)"
echo ""
echo "PROXIMOS PASSOS:"
echo "  1. Acesse o inventario e cadastre usuario/senha SSH nos dispositivos"
echo "  2. Va em Terminal e conecte nos dispositivos"
echo "  3. Digite comandos em tempo real!"
echo ""
